...
static inline auto wrap_cptr( T* ptr, Releaser&& r ) noexcept
...
///          as its first and only parameter.
template <typename T, typename Releaser>
static inline auto wrap_carray( T* ptr, Releaser&& r ) noexcept
    -> std::unique_ptr<T[], typename std::decay<decltype( r )>::type>
{
    return std::unique_ptr<T[], typename std::decay<decltype( r )>::type>{
        ptr, std::forward<Releaser>( r )
...
///
template <typename T>
static inline std::unique_ptr<T, void (*)(void*)> wrap_cptr( T* ptr ) noexcept
{
...
///
template <typename T>
static inline std::unique_ptr<T[], void (*)(void*)> wrap_carray( T* ptr ) noexcept
{
