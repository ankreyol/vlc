...
#define DISABLE_PASSTHROUGH_TEXT N_( "Disable Audio passthrough" )
#define DISABLE_PASSTHROUGH_LONGTEXT N_( "Disable if your receiver does not support Dolby/DTS" )
#define DISABLE_MULTICHANNEL_TEXT N_( "Disable multichannel PCM" )
#define DISABLE_MULTICHANNEL_LONGTEXT N_( "Disable use of PCM for multicannel audio." )
...
vlc_module_begin ()
    ...
    add_bool(SOUT_CFG_PREFIX "disable-audio-passthrough", false, DISABLE_PASSTHROUGH_TEXT, DISABLE_PASSTHROUGH_LONGTEXT, false )
    add_bool(SOUT_CFG_PREFIX "disable-multichannel-pcm", false, DISABLE_MULTICHANNEL_TEXT, DISABLE_MULTICHANNEL_LONGTEXT, false );
 ...
 *
 * Additionally:
 * - Allow (E)AC3 passthrough depending on the disable-audio-passthrough
                                        ...
                                        const audio_format_t* p_fmt ) const
{
    ...
    {
        return !var_InheritBool( p_stream, SOUT_CFG_PREFIX "disable-audio-passthrough" );
    }
                                                      ...
                                                      const audio_format_t* p_fmt )
{
    if ( p_fmt->i_channels > 2 &&
         !var_InheritBool( p_stream, SOUT_CFG_PREFIX "disable-multichannel-pcm" ) )
