...
static const unsigned int MAX_TRANSCODE_PASS = 4;
...
/**
 * Transcode strategy:
 * 0: Accept HEVC/VP9 & multichannel AAC
 * 1: Accept HEVC/VP9 & fallback to stereo MP3/multichannel PCM
 * 2: Accept multichannel AAC & transcode to H264 (in case the video codec was the culprit)
 * 3: Transcode to H264 + MP3/multichannel PCM
 *
 * Additionally, allow (E)AC3 passthrough depending on the audio-passthrough
 * config value, except for the final step, where we just give up and transcode
 * everything.
 *
 * For all attempts, accept formats that are always supported by the chromecast
 * https://developers.google.com/cast/docs/media
 */

bool sout_stream_sys_t::canDecodeVideo( vlc_fourcc_t i_codec ) const
{
    if ( transcode_attempt_idx <= 1 )
    {
                                        ...
                                        const audio_format_t* p_fmt ) const
{
    ...
    {
        bool b_passthrough = var_InheritBool( p_stream, SOUT_CFG_PREFIX "audio-passthrough" );
        return b_passthrough && transcode_attempt_idx < MAX_TRANSCODE_PASS - 1;
    }
            ...
            i_codec == VLC_CODEC_MP4A )
    {
        if ( p_fmt->i_channels > 2 )
            return transcode_attempt_idx == 0 || transcode_attempt_idx == 2;
        return true;
    }
...
bool sout_stream_sys_t::UpdateOutput( sout_stream_t *p_stream )
{
    ...
    const audio_format_t *p_original_audio = NULL;
